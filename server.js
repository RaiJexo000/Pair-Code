...
function saveSessionData(sessionId, data) {
    try {
        ...
        console.log('⚠️ Could not save session data:', e.message);
    }
}
...
function loadSessionData(sessionId) {
    try {
        ...
        console.log('⚠️ Could not load session data:', e.message);
    }
    return null;
}
...
app.post('/api/generate_pair', async (req, res) => {
    ...
    try {
        ...
        sessions[sessionId] = { sock, saveCreds, phone: finalPhone, connected: false, jid: null };
        ...
        sock.ev.on('connection.update', (update) => {
            if (update.connection === 'open') {
                ...
                // ✅ FIX: Store JID properly
                const userJid = sock.user?.id || 'status@broadcast';
                sessions[sessionId].jid = userJid;
...
// ==================== SEND COMMAND WITH PROCESSING MESSAGE ====================
...
app.post('/api/send', async (req, res) => {
    ...
    if (!sock || !jid) {
        return res.json({ success: false, message: 'Session not ready. JID missing.' });
    }

    // Extract subject for processing message
    let subject = command;
    if (command.includes(' ')) {
        subject = command.split(' ').slice(1).join(' ');
    }

    // ✅ FIX: Send processing message (for non-menu commands)
    if (!command.startsWith('.menu')) {
        try {
            await sock.sendMessage(jid, { 
                text: `⏳ Processing For ${subject || command}...` 
            });
            console.log(`✅ Processing message sent: Processing For ${subject || command}...`);
        } catch (e) {
            console.log('⚠️ Could not send processing message:', e.message);
        }
    }
