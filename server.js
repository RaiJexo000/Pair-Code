...
async function downloadMovieVideo(movieName) {
    try {
        const searchResult = await ytSearch(`${movieName} movie trailer official`);
        const video = searchResult.videos[0];
        if (!video) return { error: 'No video found' };
        const movieDetails = await lookupMovie(movieName);
        const videoPath = await downloadVideoFile(video.url, movieName);
        const fileSize = fs.existsSync(videoPath) ? fs.statSync(videoPath).size / (1024 * 1024) : 'N/A';
        return {
            title: movieDetails.Title || movieName,
            ...
            poster: movieDetails.Poster || video.thumbnail,
            videoPath: videoPath,
            fileSize: typeof fileSize === 'number' ? fileSize.toFixed(2) : 'N/A'
        };
    } catch (error) {
...
async function downloadAnimeEpisode(animeName) {
    try {
        const searchResult = await ytSearch(`${animeName} episode 1`);
        const video = searchResult.videos[0];
        if (!video) return { error: 'No video found' };
        const animeDetails = await lookupAnime(animeName);
        const videoPath = await downloadVideoFile(video.url, animeName);
        const fileSize = fs.existsSync(videoPath) ? fs.statSync(videoPath).size / (1024 * 1024) : 'N/A';
        return {
            title: animeDetails?.title || animeName,
            score: animeDetails?.score || 'N/A',
            type: animeDetails?.type || 'N/A',
            episodes: animeDetails?.episodes || 'N/A',
            synopsis: animeDetails?.synopsis || 'N/A',
            genres: animeDetails?.genres?.map(g => g.name).join(', ') || 'N/A',
            image: animeDetails?.images?.jpg?.image_url || video.thumbnail,
            videoPath: videoPath,
            fileSize: typeof fileSize === 'number' ? fileSize.toFixed(2) : 'N/A'
        };
    } catch (error) {
...
function getEmojiForMessage(text) {
    ...
    if (msg.match(/\b(hi|hello|hey|namaste|hola)\b/)) return '👋';
    if (msg.match(/\b(good morning|gm|morning)\b/)) return '🌅';
    if (msg.match(/\b(good night|gn|night)\b/)) return '🌙';
    if (msg.match(/\b(good evening|ge|evening)\b/)) return '🌆';
    if (msg.match(/\b(how are you|how r u|what's up|sup)\b/)) return '😊';
    if (msg.match(/\b(thank you|thanks|thnx|ty|thanku)\b/)) return '🙏';
    if (msg.match(/\b(sorry|so sorry|my bad)\b/)) return '😔';
    if (msg.match(/\b(bye|goodbye|see you|cya|tata)\b/)) return '👋';
    if (msg.match(/\b(ok|okay|k|alright|fine)\b/)) return '👍';
    if (msg.match(/\b(no|nah|never|not|nope)\b/)) return '❌';
    if (msg.match(/\b(yes|yeah|yep|sure|okay)\b/)) return '✅';
    if (msg.match(/\b(wow|amazing|awesome|great|super)\b/)) return '🤩';
    if (msg.match(/\b(haha|lol|rofl|lmfao|funny)\b/)) return '😂';
    if (msg.match(/\b(sad|cry|unhappy|depressed)\b/)) return '😢';
    if (msg.match(/\b(love|❤️|heart|kiss|crush)\b/)) return '❤️';
    if (msg.match(/\b(happy|glad|joy|cheerful)\b/)) return '😄';
    if (msg.match(/\b(cool|awesome|nice|good|fine)\b/)) return '😎';
    if (msg.match(/\b(food|hungry|eat|lunch|dinner)\b/)) return '🍔';
    if (msg.match(/\b(weather|rain|sun|cloud|hot|cold)\b/)) return '🌤️';
    if (msg.match(/\b(music|song|sing|guitar)\b/)) return '🎵';
    if (msg.match(/\b(movie|film|cinema|show|series)\b/)) return '🎬';
    if (msg.match(/\b(book|read|novel|page)\b/)) return '📖';
    if (msg.match(/\b(gym|workout|exercise|fitness)\b/)) return '💪';
    if (msg.match(/\b(sleep|bed|tired|rest)\b/)) return '😴';
    if (msg.match(/\b(home|house|room|door)\b/)) return '🏠';
    if (msg.match(/\b(car|drive|vehicle|bike)\b/)) return '🚗';
    if (msg.match(/\b(phone|mobile|call|text)\b/)) return '📱';
    if (msg.match(/\b(study|learn|class|school|college)\b/)) return '📚';
    if (msg.match(/\b(work|job|office|busy)\b/)) return '💼';
    if (msg.match(/\b(party|dance|celebrate|fun)\b/)) return '🎉';
    if (msg.match(/\b(help|support|assist|save)\b/)) return '🆘';
    if (msg.match(/\b(birthday|happy birthday)\b/)) return '🎂';
    return '🤖';
}
...
app.post('/api/generate_pair', async (req, res) => {
    ...
    try {
        ...
        sessions[sessionId] = { sock, saveCreds, phone: finalPhone, connected: false, jid: null, settings: {} };
